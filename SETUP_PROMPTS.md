...
제품 주제는 **"TUI로 사용자를 열받게 하는 서비스 만들기"**로 고정한다.
소프트웨어 마에스트로 연수생 또는 개발자가 간단한 개발 작업을 입력하면,
실행하는 대신 황당한 선행 작업 3개가 늘어나는 야크 셰이빙 TUI다.
결과 화면에는 작업 트리, 분노 게이지, 예상 퇴근 시간을 표시한다.
다른 제품 주제를 제안하거나 웹 앱으로 바꾸지 마라.
